...
import { MaterialReducer } from "./lib/MaterialReducer.js";
import { ProxyBatchedMesh } from "./lib/ProxyBatchedMesh.js";
...
function init() {
	...
	new GLTFLoader().load(
		"https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/mars-site/scene.gltf",
		(gltf) => {
	...
	controller.addEventListener("selectend", () => {
		if (selectedGoalIndex !== -1) {
			...
			if (
				startPos.distanceTo(endPos) < 1e-2 / params.scale &&
				window.performance.now() - startTime < 500.0
			) {
				deleteGoal(goal);
			}
...
function raycast(justGoals = false) {
	...
	intersectGoals.length =
		intersectGoals.length < goals.length ? intersectGoals.length : goals.length;
...
function rebuildGUI() {
	...
	gui
		.add(params, "model", ["ATHLETE", "Robonaut", "Curiosity", "Staubli"])
		.onChange((value) => {
	...
	solveFolder
		.add(solverOptions, "maxIterations")
		.min(1)
		.max(10)
		.step(1)
		.listen();
	solveFolder
		.add(solverOptions, "divergeThreshold")
		.min(0)
		.max(0.5)
		.step(1e-2)
		.listen();
	solveFolder
		.add(solverOptions, "stallThreshold")
		.min(0)
		.max(0.01)
		.step(1e-4)
		.listen();
	solveFolder
		.add(solverOptions, "translationErrorClamp")
		.min(1e-2)
		.max(1)
		.listen();
	solveFolder
		.add(solverOptions, "rotationErrorClamp")
		.min(1e-2)
		.max(1)
		.listen();
	solveFolder
		.add(solverOptions, "translationConvergeThreshold")
		.min(1e-3)
		.max(1e-1)
		.listen();
	solveFolder
		.add(solverOptions, "rotationConvergeThreshold")
		.min(1e-5)
		.max(1e-2)
		.listen();
	solveFolder
		.add(solverOptions, "restPoseFactor")
		.min(0)
		.max(0.25)
		.step(1e-2)
		.listen();
			...
			if (c.isMesh) {
				...
				if (c.geometry.attributes.uv && !c.material.map) {
					...
					c.geometry.setAttribute(
						"uv",
						new BufferAttribute(new Float32Array(count * 2), 2, false)
					);
				}
			}
		});
