...
function init() {
	...
	const dimension = 400;
	terrain = new Mesh(
		new PlaneGeometry(25, 25, dimension, dimension),
	...
	gui
		.add(params, "settleIterations")
		.min(1)
		.max(20)
		.step(1)
		.onChange(() => (ikNeedsUpdate = true));
	gui
		.add(params, "terrainHeight", 0.05, 0.7)
		.onChange(() => (ikNeedsUpdate = true));
	...
	loader
		.loadAsync(
			"https://raw.githubusercontent.com/gkjohnson/m2020-urdf-models/main/rover/m2020.urdf"
		)
		.then((result) => {
			result.traverse((c) => {
			...
			const differential = ikRoot.find(
				(c) => c.name === "CENTER_DIFFERENTIAL"
			).child;
			differential.removeChild(differential.children[0]);

			// left connector
			const leftDifferential = ikRoot.find(
				(c) => c.name === "LEFT_DIFFERENTIAL"
			).child;
			...
			// right connector
			const rightDifferential = ikRoot.find(
				(c) => c.name === "RIGHT_DIFFERENTIAL"
			).child;
...
function updateIk() {
	...
	for (let i = 0; i < params.settleIterations; i++) {
		...
		solveOutput +=
			delta.toFixed(2) + "ms " + SOLVE_STATUS_NAMES[results[0]] + "\n";
