...
function init() {
	...
	renderer.domElement.addEventListener("pointerup", (e) => {
		if (Math.abs(e.clientX - mouse.x) > 3 || Math.abs(e.clientY - mouse.y) > 3)
			return;
		...
		if (e.button === 2) {
			...
			if (!transformControls.dragging) {
				selectedGoalIndex = goalIcons.indexOf(
					result ? result.object.parent : null
				);
...
function render() {
	...
	if (ikRoot) {
		...
		if (params.solve) {
			...
			outputContainer.innerText += `avg solve time \t: ${averageTime.toFixed(
				3
			)}ms\n`;
			outputContainer.innerText += statuses
				.map((s) => SOLVE_STATUS_NAMES[s])
				.join("\n");
