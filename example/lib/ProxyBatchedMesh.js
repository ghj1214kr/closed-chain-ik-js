...
export class ProxyBatchedMesh extends Group {
	...
	constructor(root) {
		...
		if (root.parent) {
			throw new Error(
				"ProxyBatchedMesh : Proxied root is not expected to have a parent."
			);
		}
		...
		root.traverse((c) => {
			if (c.isMesh) {
				if (Array.isArray(c.material)) {
					...
					const geometry = hadIndex
						? c.geometry.clone().toNonIndexed()
						: c.geometry;
		...
		materialToGeometry.forEach((infoArray, material) => {
			...
			const skinnedMesh = new ProxySkinnedMesh(
				mergedGeometry,
				material,
				Array.from(meshSet)
			);
	...
	updateMatrixWorld(...args) {
		...
		if (proxied.parent && proxied.parent !== this) {
			console.warn(
				"ProxyBatchedMesh : Proxy mesh is expected to not have parent."
			);
		}
