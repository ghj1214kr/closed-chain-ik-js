...
export function accumulateClosureError(
	...
	const { translationDoFCount, rotationDoFCount, dofFlags, dof } = joint;
...
export function accumulateTargetError(
	...
	result.isConverged =
		posDelta < translationConvergeThreshold &&
		rotDelta < rotationConvergeThreshold;
	...
	if (errorVector) {
		...
		vec3.scale(
			tempPos,
			tempPos,
			(translationFactor * translationErrorClamp) / posMag
		);
		...
		vec3.scale(
			tempEuler,
			tempEuler,
			(rotationFactor * rotationErrorClamp) / eulerMag
		);
		for (
			let i = translationDoFCount, l = translationDoFCount + rotationDoFCount;
			i < l;
			i++
		) {
			const dof = dofList[i];
