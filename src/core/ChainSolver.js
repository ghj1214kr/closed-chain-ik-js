...
import {
	accumulateClosureError,
	accumulateTargetError,
...
export const SOLVE_STATUS_NAMES = Object.entries(SOLVE_STATUS)
	.sort((a, b) => a[1] - b[1])
	.map((el) => el[0]);
...
export class ChainSolver {
		...
		do {
			...
			if (restPoseFactor !== 0) {
				...
				for (let i = 0, l = freeJoints.length; i < l; i++) {
					...
					const colCount =
						joint.rotationDoFCount + joint.translationDoFCount - lockedDoFCount;
	...
	applyJointAngles(freeJoints, deltaTheta) {
		const { lockedJointDoF, lockedJointDoFCount } = this;
	...
	fillJacobian(targetJoints, freeJoints, outJacobian) {
		...
		for (let c = 0, tc = freeJoints.length; c < tc; c++) {
			...
			const relevantConnectedClosures =
				affectedConnectedClosures.get(freeJoint);
			const dofList = freeJoint.dof;
			const colCount =
				freeJoint.translationDoFCount + freeJoint.rotationDoFCount;
			...
			for (let co = 0; co < colCount; co++) {
				...
				for (let r = 0, tr = targetJoints.length; r < tr; r++) {
					...
					if (targetJoint.isClosure) {
						if (
							relevantClosures.has(targetJoint) ||
							relevantConnectedClosures.has(targetJoint)
						) {
							...
							if (relevantConnectedClosures.has(targetJoint)) {
								...
								mat4.multiply(
									...
									tempInverseMatrixWorld,
									targetJoint.child.matrixWorld
								);
								mat4.multiply(
									targetDeltaWorldMatrix,
									tempDeltaWorldMatrix,
									targetRelativeToJointMatrix
								);

								// Get the new error
								getMatrixDifference(
									targetJoint.matrixWorld,
									targetDeltaWorldMatrix,
									tempPos2,
									tempQuat2
								);
							...
							if (targetJoint.isGoal) {
								const { translationDoFCount, rotationDoFCount, dof } =
									targetJoint;
								...
								if (rotationDoFCount === 3) {
									outJacobian[rowIndex + translationDoFCount + 0][colIndex] =
										tempQuat[0];
									outJacobian[rowIndex + translationDoFCount + 1][colIndex] =
										tempQuat[1];
									outJacobian[rowIndex + translationDoFCount + 2][colIndex] =
										tempQuat[2];
									outJacobian[rowIndex + translationDoFCount + 3][colIndex] =
										tempQuat[3];
									rowIndex += 4;
								}
					...
					if (targetJoint.targetSet) {
						const rowCount =
							targetJoint.translationDoFCount + targetJoint.rotationDoFCount;
