...
export const DOF_NAMES = Object.entries(DOF)
	.sort((a, b) => a[1] - b[1])
	.map((e) => e[0]);
...
export class Joint extends Frame {
	...
	_setValue(target, dof, value) {
		if (target === this.minDoFLimit || target == this.maxDoFLimit) {
			throw new Error(
				"Joint: Cannot set minDoFLimit or maxDoFLimit with _setValue."
			);
		}
	...
	setDoF(...args) {
		args.forEach((dof, i) => {
			...
			if (args.includes(dof, i + 1)) {
				throw new Error(
					"Joint: Duplicate degree of freedom " + DOF_NAMES[dof] + "specified."
				);
			}

			if (i !== 0 && args[i - 1] > dof) {
				throw new Error(
					"Joint: Joints degrees of freedom must be specified in position then rotation, XYZ order"
				);
			}
		});
		...
		this.rotationDoFCount =
			this.dofFlags[DOF.EX] + this.dofFlags[DOF.EY] + this.dofFlags[DOF.EZ];
		this.translationDoFCount =
			this.dofFlags[DOF.X] + this.dofFlags[DOF.Y] + this.dofFlags[DOF.Z];
	}
		...
		if (!trackJointWrap) {
			if (rotationDoFCount < 3) {
				for (let i = DOF.EX; i <= DOF.EZ; i++) {
					dofTarget[i] = toSmallestEulerValueDistance(
						dofValues[i],
						dofTarget[i]
					);
					dofRestPose[i] = toSmallestEulerValueDistance(
						dofValues[i],
						dofRestPose[i]
					);
				}
	...
	getDeltaWorldMatrix(dof, delta, outMatrix) {
		...
		const doInvert =
			(isMaxConstrained && minSlack > maxSlack) ||
			(isMinConstrained && maxSlack > minSlack);
		if (doInvert) {
	...
	removeChild(child) {
		if (this.isClosure) {
			if (this.child !== child) {
				throw new Error(
					"Frame: Child to be removed is not a child of this Joint."
				);
			} else {
				this.child = null;
				this.isClosure = false;
